...
<style>
    ...
    .mbti-hero { font-size: 65px; font-weight: 900; margin: 10px 0; text-shadow: 0 4px 10px rgba(0,0,0,0.3); letter-spacing: 3px; color: #FFFFFF; }
    ...
    .rx-badge { display: inline-block; background: #FFF1F2; color: #E11D48; font-size: 11px; padding: 2px 6px; border-radius: 4px; font-weight: bold; margin-left: 5px; border: 1px solid #FECDD3; }
    .stock-badge { display: inline-block; background: #F0FDF4; color: #16A34A; font-size: 11px; padding: 2px 6px; border-radius: 4px; font-weight: bold; margin-left: 5px; border: 1px solid #BBF7D0; }
    ...
    .sales-guide-box { background-color: #F0F9FF; border: 1px solid #BAE6FD; padding: 15px; border-radius: 10px; margin-top: 20px; text-align: left; }
    .sales-guide-title { font-weight: bold; color: #0284C7; font-size: 14px; margin-bottom: 5px; }
    .sales-guide-text { font-size: 13px; color: #334155; line-height: 1.6; }
...
def get_dictionary_data(category):
    if category == 'glasses':
        return pd.DataFrame([
            {'id': 1, 'brand': '케미', 'name': '퍼펙트 UV', 'price': 30000, 'price_table': {'1.56': 30000, '1.60': 60000}, 'img': 'https://via.placeholder.com/300x200?text=CHEMI+Perfect+UV', 'spec_design': '비구면', 'spec_material': 'MR-8', 'spec_coat': 'Perfect UV', 'spec_uv': 'UV400', 'desc': '자외선 99.9% 및 블루라이트 차단 가성비 렌즈.', 'tags': ['#가성비', '#청광차단'], 'thin': 6, 'view': 6, 'coat': 5},
            {'id': 2, 'brand': '니콘', 'name': 'BLUV Plus', 'price': 60000, 'price_table': {'1.56': 60000, '1.60': 90000}, 'img': 'https://via.placeholder.com/300x200?text=NIKON+BLUV', 'spec_design': '양면 UV', 'spec_material': 'Nikon', 'spec_coat': 'SeeCoat', 'spec_uv': '양면차단', 'desc': '후면 반사 자외선 차단 및 디지털 피로 완화.', 'tags': ['#디지털케어', '#양면차단'], 'thin': 7, 'view': 7, 'coat': 7},
            {'id': 3, 'brand': '호야', 'name': '뉴럭스', 'price': 70000, 'price_table': {'1.60': 70000, '1.67': 110000}, 'img': 'https://via.placeholder.com/300x200?text=HOYA+Nulux', 'spec_design': 'Trueform', 'spec_material': 'Eyas 1.60', 'spec_coat': 'VG', 'spec_uv': 'UV Ban', 'desc': '스크래치에 강한 고강도 코팅.', 'tags': ['#흠집방지', '#고강도'], 'thin': 7, 'view': 8, 'coat': 9},
            {'id': 4, 'brand': '케미', 'name': '양면비구면 D-Free', 'price': 80000, 'price_table': {'1.60': 80000, '1.67': 110000}, 'img': 'https://via.placeholder.com/300x200?text=CHEMI+D-Free', 'spec_design': '양면비구면', 'spec_material': 'MR-8', 'spec_coat': 'Aegis', 'spec_uv': 'UV400', 'desc': '주변부 왜곡 최소화.', 'tags': ['#미용효과', '#난시교정'], 'thin': 8, 'view': 8, 'coat': 7},
            {'id': 5, 'brand': '자이스', 'name': '클리어뷰', 'price': 100000, 'price_table': {'1.60': 100000, '1.67': 140000}, 'img': 'https://via.placeholder.com/300x200?text=ZEISS+ClearView', 'spec_design': 'Freeform', 'spec_material': 'Zeiss', 'spec_coat': 'Platinum', 'spec_uv': 'UVProtect', 'desc': '3배 더 넓은 선명한 시야.', 'tags': ['#초선명', '#자이스'], 'thin': 8, 'view': 9, 'coat': 8},
            {'id': 6, 'brand': '에실로', 'name': '트랜지션스 Gen8', 'price': 150000, 'price_table': {'1.50': 150000, '1.60': 220000}, 'img': 'https://via.placeholder.com/300x200?text=Transitions', 'spec_design': '변색', 'spec_material': 'Orma', 'spec_coat': 'Sapphire', 'spec_uv': 'UV400', 'desc': '실내 투명, 실외 선글라스 자동 변색.', 'tags': ['#변색렌즈', '#패션'], 'thin': 7, 'view': 8, 'coat': 8},
            {'id': 7, 'brand': '토카이', 'name': '루티나', 'price': 180000, 'price_table': {'1.60': 180000, '1.76': 400000}, 'img': 'https://via.placeholder.com/300x200?text=TOKAI+Lutina', 'spec_design': '비구면', 'spec_material': 'Lutina', 'spec_coat': 'ESC', 'spec_uv': 'HEV', 'desc': '루테인 보호, 망막 건강.', 'tags': ['#눈건강', '#망막보호'], 'thin': 9, 'view': 9, 'coat': 10},
            {'id': 8, 'brand': '자이스', 'name': '드라이브세이프', 'price': 250000, 'price_table': {'1.50': 250000, '1.60': 360000}, 'img': 'https://via.placeholder.com/300x200?text=ZEISS+DriveSafe', 'spec_design': 'Luminance', 'spec_material': 'Zeiss', 'spec_coat': 'DriveSafe', 'spec_uv': 'UVProtect', 'desc': '야간 운전 눈부심 감소.', 'tags': ['#야간운전', '#안전운전'], 'thin': 8, 'view': 10, 'coat': 9}
        ])
    else:
        return pd.DataFrame([
            ...
            {'id': 105, 'brand': '아큐브', 'name': '오아시스 원데이', 'price': 63000, 'qty': '30p', 'img': 'https://via.placeholder.com/300x200?text=Oasys', 'spec_mat': 'Senofilcon A', 'spec_water': '38%', 'spec_dk': '121', 'spec_bc': '8.5', 'desc': '전 세계 베스트셀러.', 'tags': ['#베스트셀러'], 'dry': 8, 'handle': 8, 'oxygen': 9},
            {'id': 106, 'brand': '알콘', 'name': '데일리스 토탈원', 'price': 69000, 'qty': '30p', 'img': 'https://via.placeholder.com/300x200?text=Total1', 'spec_mat': 'Delefilcon A', 'spec_water': '33%', 'spec_dk': '156', 'spec_bc': '8.5', 'desc': '워터렌즈, 건조감 해결.', 'tags': ['#강소라렌즈'], 'dry': 10, 'handle': 4, 'oxygen': 10},
            {'id': 107, 'brand': '알콘', 'name': '토탈원 난시', 'price': 79000, 'qty': '30p', 'img': 'https://via.placeholder.com/300x200?text=Total1+Toric', 'spec_mat': 'Delefilcon A', 'spec_water': '33%', 'spec_dk': '127', 'spec_bc': '8.6', 'desc': '토탈원의 난시 교정 버전.', 'tags': ['#난시교정'], 'dry': 10, 'handle': 4, 'oxygen': 9},
    ...
    if mode == 'glasses':
        ...
        data = [
            {'id': 1, 'brand': '케미', 'name': '퍼펙트 UV', 'base_price': 30000, 'cat': 'general', 'tier': 0, 'view': 6, 'coat': 5, 'tags': ['#블루라이트차단', '#가성비'], 
             'type': '여벌', 'ai_summary': '자외선 99.9% 차단과 블루라이트 부분 차단 기능이 들어간 <b>퍼펙트 UV</b> 기술이 적용된 실속형 렌즈입니다.'},
            {'id': 2, 'brand': '니콘', 'name': 'BLUV Plus', 'base_price': 60000, 'cat': 'digital', 'tier': 1, 'view': 7, 'coat': 7, 'tags': ['#양면차단', '#디지털피로'],
             'type': '여벌', 'ai_summary': '니콘만의 <b>이중 자외선 차단</b> 및 디지털 눈 피로 완화 존이 설계되어 있어 근거리 작업이 편안합니다.'},
            {'id': 3, 'brand': '호야', 'name': '뉴럭스', 'base_price': 70000, 'cat': 'general', 'tier': 1, 'view': 8, 'coat': 9, 'tags': ['#흠집방지', '#선명함'],
             'type': '여벌', 'ai_summary': '호야의 <b>VG(Venus Guard) 코팅</b>이 적용되어 일반 렌즈 대비 스크래치에 5배 강하고 먼지가 덜 붙습니다.'},
            {'id': 4, 'brand': '케미', 'name': '양면비구면 D-Free', 'base_price': 80000, 'cat': 'distortions', 'tier': 1, 'view': 8, 'coat': 7, 'tags': ['#왜곡최소화', '#넓은시야'],
             'type': 'RX주문' if abs(sph) > 8 or abs(cyl) > 2 else '여벌', 
             'ai_summary': '<b>양면 비구면(D-Free)</b> 기술로 주변부 왜곡을 줄여 눈이 편안하며, 페이스 라인 축소 현상을 완화합니다.'},
            {'id': 5, 'brand': '자이스', 'name': '클리어뷰', 'base_price': 100000, 'cat': 'general', 'tier': 2, 'view': 9, 'coat': 8, 'tags': ['#초선명', '#얇은두께'],
             'type': '여벌', 'ai_summary': '자이스의 <b>클리어뷰 프리폼 기술</b>로 렌즈 중심부터 주변부까지 3배 더 넓은 선명한 시야를 제공합니다.'},
            {'id': 6, 'brand': '에실로', 'name': '트랜지션스 Gen8', 'base_price': 150000, 'cat': 'outdoor', 'tier': 2, 'view': 8, 'coat': 8, 'tags': ['#변색렌즈', '#선글라스'],
             'type': 'RX주문', 'ai_summary': '<b>Gen8 변색 기술</b>로 실내에선 투명하고 실외에선 선글라스처럼 빠르게 변하는 2-in-1 렌즈입니다.'},
            {'id': 7, 'brand': '토카이', 'name': '루티나', 'base_price': 180000, 'cat': 'premium', 'tier': 3, 'view': 9, 'coat': 10, 'tags': ['#눈건강', '#망막보호'],
             'type': 'RX주문', 'ai_summary': '산화 스트레스를 유발하는 파장을 차단하는 <b>루티나 소재</b>로 눈 속 루테인을 보호하는 헬스케어 렌즈입니다.'},
            {'id': 8, 'brand': '자이스', 'name': '드라이브세이프', 'base_price': 250000, 'cat': 'drive', 'tier': 3, 'view': 10, 'coat': 9, 'tags': ['#야간운전', '#빛번짐차단'],
             'type': 'RX주문', 'ai_summary': '<b>루미넌스 디자인</b>으로 동공 크기를 반영해 야간 빛 번짐을 줄이고 악천후에도 선명함을 유지합니다.'}
        ]
        ...
        # 콘택트렌즈
        data = [
            {'id': 101, 'brand': '미광', 'name': '클리어 원데이', 'category': 'sphere', 'tier': 0, 'price': 32000, 'dry_score': 4, 'dkt': 25, 'handling': 9, 'oxygen': 3, 'tags': ['#가성비갑'], 'type': '팩렌즈', 'ai_summary': '<b>Hioxifilcon A</b> 재질로 초기 착용감이 촉촉하며, 가격 부담 없이 매일 쓰기 좋은 가성비 제품입니다.'},
            {'id': 102, 'brand': '쿠퍼비전', 'name': '클래리티 원데이', 'category': 'sphere', 'tier': 1, 'price': 45000, 'dry_score': 7, 'dkt': 86, 'handling': 7, 'oxygen': 8, 'tags': ['#실리콘'], 'type': '팩렌즈', 'ai_summary': '눈이 숨 쉴 수 있는 <b>실리콘 하이드로겔</b> 소재를 합리적인 가격에 제공하여 눈 충혈을 예방합니다.'},
            {'id': 106, 'brand': '알콘', 'name': '데일리스 토탈원', 'category': 'sphere', 'tier': 3, 'price': 69000, 'dry_score': 10, 'dkt': 156, 'handling': 4, 'oxygen': 10, 'tags': ['#건조감종결'], 'type': '팩렌즈', 'ai_summary': '표면 함수율 80% 이상의 <b>워터 그라디언트</b> 기술로 눈꺼풀 마찰을 최소화해 렌즈를 안 낀 듯 편안합니다.'},
            {'id': 105, 'brand': '아큐브', 'name': '오아시스 원데이', 'category': 'sphere', 'tier': 2, 'price': 63000, 'dry_score': 8, 'dkt': 121, 'handling': 8, 'oxygen': 9, 'tags': ['#베스트셀러'], 'type': '팩렌즈', 'ai_summary': '눈물 유사 성분의 <b>하이드라럭스</b> 기술로 디지털 기기 사용 시 건조감을 줄여주는 베스트셀러입니다.'},
            {'id': 103, 'brand': '인터로조', 'name': '오투오투 원데이', 'category': 'sphere', 'tier': 1, 'price': 45000, 'dry_score': 7, 'dkt': 130, 'handling': 8, 'oxygen': 9, 'tags': ['#국산'], 'type': '팩렌즈', 'ai_summary': '130 Dk/t의 <b>높은 산소전달률</b>을 자랑하는 국산 프리미엄 렌즈로 가성비와 눈 건강을 모두 잡았습니다.'},
            {'id': 104, 'brand': '바슈롬', 'name': '울트라 원데이', 'category': 'sphere', 'tier': 2, 'price': 55000, 'dry_score': 8, 'dkt': 134, 'handling': 8, 'oxygen': 9, 'tags': ['#촉촉함'], 'type': '팩렌즈', 'ai_summary': '<b>모이스처 씰</b> 기술로 16시간 착용 후에도 렌즈 수분의 96%를 유지하여 늦은 저녁까지 촉촉합니다.'},
            {'id': 107, 'brand': '알콘', 'name': '토탈원 난시', 'category': 'toric', 'tier': 3, 'price': 79000, 'dry_score': 10, 'dkt': 127, 'handling': 4, 'oxygen': 9, 'tags': ['#난시교정'], 'type': '팩렌즈', 'ai_summary': '워터 그라디언트 재질에 <b>프리시전 밸런스 8/4</b> 디자인을 더해, 건조감 없이 선명한 난시 교정을 제공합니다.'},
            {'id': 108, 'brand': '아큐브', 'name': '오아시스 난시', 'category': 'toric', 'tier': 2, 'price': 74000, 'dry_score': 8, 'dkt': 121, 'handling': 8, 'oxygen': 9, 'tags': ['#축안정'], 'type': '팩렌즈', 'ai_summary': '눈의 깜빡임을 이용한 <b>ASD 기술</b>로 눕거나 운동할 때도 축이 돌아가지 않아 선명함을 유지합니다.'}
        ]
...
q_labels = {
    ...
    'sen_1': 'Q6.오후 시간대 눈 뻑뻑함/충혈 발생', 'sen_2': 'Q7.렌즈 착용 시 이물감 예민하게 느낌', 
    'sen_3': 'Q8.근거리(폰/독서) 작업 시 초점 흐림/피로', 'sen_4': 'Q9.눈 시림 및 따가움 자주 느낌', 'sen_5': 'Q10.난시로 인한 글자 번짐/흐림 심함',
...
if st.session_state['page'] == 'optician_view':
    ...
    type_t = "T" if data['val'] >= 6 else "F" # 여기는 안경사 뷰라 원래 기준 유지
    ...
    with tab1:
        ...
        for rk, (idx, row) in enumerate(ranks.iterrows(), 1):
            # [여벌/RX] 태그 추가
            badge_class = "rx-badge" if "RX" in row['type'] else "stock-badge"
            st.markdown(f"<div style='background:white; padding:15px; border-radius:10px; border:1px solid #eee; margin-bottom:10px;'><div style='font-weight:bold; font-size:16px;'>{rk}위. {row['name']} <span class='{badge_class}'>{row['type']}</span></div><div style='color:#666; font-size:13px;'>{row['brand']} | {format(int(row['final_price']),',')}원 (권장소비자가)</div></div>", unsafe_allow_html=True)
    ...
    # [직관적 상담 가이드 - 피드백 반영]
    # MBTI와 점수를 바탕으로 상담 멘트 생성
    guide_text = ""
    if data['val'] >= 7: guide_text += "• 💎 <b>성능/품질 최우선:</b> 가격대가 있더라도 눈이 편안하고 기능이 좋은 프리미엄 제품을 권장하면 만족도가 높습니다.<br>"
    elif data['val'] <= 4: guide_text += "• 💰 <b>가성비 중시:</b> 합리적인 가격대의 실속형 제품이나 행사 상품 위주로 제안하는 것이 좋습니다.<br>"
    else: guide_text += "• ⚖️ <b>합리적 소비:</b> 가격 대비 성능이 검증된 베스트셀러 제품을 추천하세요.<br>"
    
    # Q8 근거리 체크
    try:
        q8_score = int(data['answers_str'][7]) # sen_3 index
        if q8_score >= 4:
            guide_text += "• 📱 <b>근거리 피로 호소:</b> 디지털 기기나 근거리 작업 시 불편함이 큽니다. <b>기능성 렌즈(피로완화)</b>나 <b>다초점 렌즈</b> 필요성을 설명해 주세요."
    except: pass

    st.markdown("---")
    st.markdown(f"""
    <div class="sales-guide-box">
        <div class="sales-guide-title">📢 안경사님을 위한 상담 가이드</div>
        <div class="sales-guide-text">
            {guide_text}
        </div>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("<div style='font-weight:bold; margin-bottom:10px; margin-top:20px; color:#2563EB;'>📊 4대 핵심 지표 분석</div>", unsafe_allow_html=True)
...
# ==============================================================================
    ...
    questions = {
        ...
        "S/N (예민도)": [("Q6. 오후가 되면 눈이 뻑뻑하신가요?", "sen_1"), ("Q7. 렌즈 이물감을 잘 느끼시나요?", "sen_2"), ("Q8. 근거리(폰/독서) 작업 시 초점이 흐리거나 피로한가요?", "sen_3"), ("Q9. 눈 시림/따가움을 자주 느끼나요?", "sen_4"), ("Q10. 난시(글자 번짐)가 심한가요?", "sen_5")],
    ...
    score_t = sum([ans[f'val_{i}'] for i in range(1,6)]); type_t = "T" if score_t >= 18 else "F" # 7.0점 기준
    ...
    with tab1:
        ...
        for rk, (idx, row) in enumerate(ranks.iterrows(), 1):
            ...
            if ans['sen_3'] >= 4: life_reasons.append("<b>근거리 피로</b>") # [NEW] 근거리 피로 반영
            ...
            if type_t == "T": val_reasons.append("<b>성능 최우선</b> 성향에 맞춰 최고 스펙 제품을 선정")
                ...
                if row['base_price'] >= 100000: val_reasons.append("가성비를 선호하시지만, <b>시력 특성상 교정력을 위해</b> 불가피하게 성능 위주로 선정")
                else: val_reasons.append("<b>가성비</b>를 최우선으로 고려하여 실속형 제품을 선정")
            ...
            with c1:
                ...
                # [NEW] 근거리 피로 경고 메시지
                warning_msg = ""
                if ans['sen_3'] >= 4 and 'digital' not in row['cat']:
                    warning_msg = "<div style='color:#E11D48; font-size:12px; font-weight:bold; margin-top:5px;'>⚠️ 근거리 피로가 높습니다. 기능성 렌즈 상담을 권장합니다.</div>"
                ...
                <div class="prod-card">
                    ...
                    <div style="font-size:20px; font-weight:800; margin-top:20px; margin-bottom:5px; color:#111;">
                        {row['name']} <span class='{badge_class}'>{row['type']}</span>
                    </div>
