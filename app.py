...
# ==============================================================================
# 1. 디자인 (CSS) - v9.1 (v8.8 기반) 스타일 유지 + 뱃지 스타일 추가
# ==============================================================================
st.markdown("""
<style>
    ...
    /* [NEW] 여벌/RX 뱃지 스타일 */
    .badge-stock { display: inline-block; background-color: #DCFCE7; color: #166534; font-size: 11px; font-weight: 800; padding: 2px 8px; border-radius: 6px; border: 1px solid #BBF7D0; vertical-align: middle; margin-left: 8px; transform: translateY(-2px); }
    .badge-rx { display: inline-block; background-color: #FEE2E2; color: #991B1B; font-size: 11px; font-weight: 800; padding: 2px 8px; border-radius: 6px; border: 1px solid #FECACA; vertical-align: middle; margin-left: 8px; transform: translateY(-2px); }
    ...
    /* [NEW] 상담 가이드 박스 */
    .sales-guide-box { background-color: #F0F9FF; border: 1px solid #BAE6FD; padding: 18px; border-radius: 12px; margin-top: 20px; text-align: left; box-shadow: 0 2px 5px rgba(0,0,0,0.02); }
    .sales-guide-title { font-weight: 800; color: #0369A1; font-size: 15px; margin-bottom: 8px; display: flex; align-items: center; gap: 6px; }
    .sales-guide-text { font-size: 13px; color: #334155; line-height: 1.7; }
...
def get_dictionary_data(category):
    if category == 'glasses':
        return pd.DataFrame([
            ...
            {'id': 2, 'brand': '니콘', 'name': 'BLUV Plus', 'price': 60000, 'price_table': {'1.56 (중굴절)': 60000, '1.60 (고굴절)': 90000, '1.67 (초고굴절)': 120000}, 'img': 'https://via.placeholder.com/300x200?text=NIKON+BLUV', 'spec_design': '양면 UV', 'spec_material': 'Nikon', 'spec_coat': 'SeeCoat', 'spec_uv': '양면차단', 'desc': '후면 반사 자외선 차단 및 디지털 피로 완화 기능을 제공합니다.', 'tags': ['#디지털케어', '#양면차단'], 'thin': 7, 'view': 7, 'coat': 7},
            ...
            {'id': 4, 'brand': '케미', 'name': '양면비구면 D-Free', 'price': 80000, 'price_table': {'1.60 (고굴절)': 80000, '1.67 (초고굴절)': 110000, '1.74 (특초)': 160000}, 'img': 'https://via.placeholder.com/300x200?text=CHEMI+D-Free', 'spec_design': '양면비구면', 'spec_material': 'MR-8', 'spec_coat': 'Aegis', 'spec_uv': 'Perfect UV', 'desc': '주변부 왜곡을 줄여 눈이 덜 작아 보임.', 'tags': ['#미용효과', '#난시교정'], 'thin': 8, 'view': 8, 'coat': 7},
            ...
            {'id': 6, 'brand': '에실로', 'name': '트랜지션스 Gen8', 'price': 150000, 'price_table': {'1.50 (일반)': 150000, '1.60 (고굴절)': 220000, '1.67 (초고굴절)': 280000}, 'img': 'https://via.placeholder.com/300x200?text=Transitions', 'spec_design': '변색', 'spec_material': 'Orma', 'spec_coat': 'Sapphire', 'spec_uv': 'UV400', 'desc': '실내 투명, 실외 선글라스 자동 변색.', 'tags': ['#변색렌즈', '#패션'], 'thin': 7, 'view': 8, 'coat': 8},
            {'id': 7, 'brand': '토카이', 'name': '루티나', 'price': 180000, 'price_table': {'1.60 (고굴절)': 180000, '1.76 (세계최초)': 400000}, 'img': 'https://via.placeholder.com/300x200?text=TOKAI+Lutina', 'spec_design': '비구면', 'spec_material': 'Lutina', 'spec_coat': 'ESC', 'spec_uv': 'HEV Cut', 'desc': '루테인 보호, 망막 건강.', 'tags': ['#눈건강', '#망막보호'], 'thin': 9, 'view': 9, 'coat': 10},
            {'id': 8, 'brand': '자이스', 'name': '드라이브세이프', 'price': 250000, 'price_table': {'1.50 (일반)': 250000, '1.60 (고굴절)': 360000, '1.67 (초고굴절)': 480000}, 'img': 'https://via.placeholder.com/300x200?text=ZEISS+DriveSafe', 'spec_design': 'Luminance', 'spec_material': 'Zeiss', 'spec_coat': 'DriveSafe', 'spec_uv': 'UVProtect', 'desc': '야간 운전 눈부심 감소 및 선명한 시야.', 'tags': ['#야간운전', '#안전운전'], 'thin': 8, 'view': 10, 'coat': 9}
        ])
    else:
        return pd.DataFrame([
            ...
            {'id': 102, 'brand': '쿠퍼비전', 'name': '클래리티 원데이', 'price': 45000, 'qty': '30p', 'img': 'https://via.placeholder.com/300x200?text=Clarity', 'spec_mat': 'Somofilcon A', 'spec_water': '56%', 'spec_dk': '86', 'spec_bc': '8.6', 'desc': '실리콘 하이드로겔 소재 가성비.', 'tags': ['#실리콘', '#가성비'], 'dry': 7, 'handle': 7, 'oxygen': 8},
            {'id': 103, 'brand': '인터로조', 'name': '오투오투 원데이', 'price': 45000, 'qty': '30p', 'img': 'https://via.placeholder.com/300x200?text=O2O2', 'spec_mat': 'Silicone', 'spec_water': '45%', 'spec_dk': '130', 'spec_bc': '8.8', 'desc': '높은 산소전달률 국산 프리미엄.', 'tags': ['#국산', '#고산소'], 'dry': 7, 'handle': 8, 'oxygen': 9},
            {'id': 104, 'brand': '바슈롬', 'name': '울트라 원데이', 'price': 55000, 'qty': '30p', 'img': 'https://via.placeholder.com/300x200?text=Ultra', 'spec_mat': 'Kalifilcon A', 'spec_water': '55%', 'spec_dk': '134', 'spec_bc': '8.6', 'desc': '16시간 촉촉함 유지.', 'tags': ['#장시간', '#촉촉함'], 'dry': 8, 'handle': 8, 'oxygen': 9},
            ...
            {'id': 106, 'brand': '알콘', 'name': '데일리스 토탈원', 'price': 69000, 'qty': '30p', 'img': 'https://via.placeholder.com/300x200?text=Total1', 'spec_mat': 'Delefilcon A', 'spec_water': '33%~80%', 'spec_dk': '156', 'spec_bc': '8.5', 'desc': '워터렌즈, 건조감 해결.', 'tags': ['#강소라렌즈', '#프리미엄'], 'dry': 10, 'handle': 4, 'oxygen': 10},
...
# [추천 데이터 - type(여벌/RX) 추가 + ai_summary(상세분석)]
    ...
    if mode == 'glasses':
        ...
        data = [
            {'id': 1, 'brand': '케미', 'name': '퍼펙트 UV', 'base_price': 30000, 'cat': 'general', 'tier': 0, 'view': 6, 'coat': 5, 'tags': ['#블루라이트차단', '#가성비'], 
             'type': '여벌', 'ai_summary': '자외선 99.9% 차단과 블루라이트 부분 차단 기능이 들어간 <b>퍼펙트 UV</b> 기술이 적용된, 가성비 최고의 실속형 렌즈입니다.'},
            {'id': 2, 'brand': '니콘', 'name': 'BLUV Plus', 'base_price': 60000, 'cat': 'digital', 'tier': 1, 'view': 7, 'coat': 7, 'tags': ['#양면차단', '#디지털피로'],
             'type': '여벌', 'ai_summary': '니콘만의 <b>이중 자외선 차단 설계</b>가 적용되어 앞뒷면 UV를 모두 막아주며, 디지털 눈 피로 완화 존이 설계되어 있습니다.'},
            {'id': 3, 'brand': '호야', 'name': '뉴럭스', 'base_price': 70000, 'cat': 'general', 'tier': 1, 'view': 8, 'coat': 9, 'tags': ['#흠집방지', '#선명함'],
             'type': '여벌', 'ai_summary': '호야의 자랑인 <b>VG(Venus Guard) 코팅</b>이 적용되어 일반 렌즈 대비 스크래치에 5배 강하고 먼지가 덜 붙습니다.'},
            {'id': 4, 'brand': '케미', 'name': '양면비구면 D-Free', 'base_price': 80000, 'cat': 'distortions', 'tier': 1, 'view': 8, 'coat': 7, 'tags': ['#왜곡최소화', '#넓은시야'],
             'type': '여벌', 'ai_summary': '렌즈의 앞면과 뒷면을 모두 평평하게 설계한 <b>양면 비구면(D-Free)</b> 기술로, 주변부 왜곡을 줄여 눈이 편안합니다.'},
            {'id': 5, 'brand': '자이스', 'name': '클리어뷰', 'base_price': 100000, 'cat': 'general', 'tier': 2, 'view': 9, 'coat': 8, 'tags': ['#초선명', '#얇은두께'],
             'type': '여벌', 'ai_summary': '자이스의 <b>클리어뷰 프리폼 기술</b>로 렌즈 중심부뿐만 아니라 주변부까지 3배 더 넓은 선명한 시야를 제공합니다.'},
            {'id': 6, 'brand': '에실로', 'name': '트랜지션스 Gen8', 'base_price': 150000, 'cat': 'outdoor', 'tier': 2, 'view': 8, 'coat': 8, 'tags': ['#변색렌즈', '#선글라스'],
             'type': 'RX (주문제작)', 'ai_summary': '전 세계 1위 <b>트랜지션스 Gen8</b> 기술로 실내에서는 투명하고 실외에서는 선글라스처럼 진하게, 빠르게 변색됩니다.'},
            {'id': 7, 'brand': '토카이', 'name': '루티나', 'base_price': 180000, 'cat': 'premium', 'tier': 3, 'view': 9, 'coat': 10, 'tags': ['#눈건강', '#망막보호'],
             'type': 'RX (주문제작)', 'ai_summary': '산화 스트레스를 유발하는 파장을 차단하는 <b>루티나(Lutina)</b> 소재를 사용하여 눈 속 루테인을 보호하는 헬스케어 렌즈입니다.'},
            {'id': 8, 'brand': '자이스', 'name': '드라이브세이프', 'base_price': 250000, 'cat': 'drive', 'tier': 3, 'view': 10, 'coat': 9, 'tags': ['#야간운전', '#빛번짐차단'],
             'type': 'RX (주문제작)', 'ai_summary': '<b>루미넌스 디자인</b> 기술로 야간에 커진 동공 크기를 반영해 빛 번짐을 줄이고 선명도를 극대화한 운전 전용 렌즈입니다.'}
        ]
...
q_labels = {
    ...
    'sen_3': 'Q8.근거리(폰/독서) 작업 시 초점이 흐리거나 피로한가요?', # [수정됨]
    'sen_4': 'Q9.눈 시림 및 따가움 자주 느낌', 'sen_5': 'Q10.난시로 인한 글자 번짐/흐림 심함',
...
if st.session_state['page'] == 'optician_view':
    ...
    with tab1:
        ...
        for rk, (idx, row) in enumerate(ranks.iterrows(), 1):
            badge_class = "badge-rx" if "RX" in row['type'] else "badge-stock"
    ...
    with tab2:
        ...
        for rk, (idx, row) in enumerate(ranks_c.iterrows(), 1):
            badge_class = "badge-rx" if "RX" in row['type'] else "badge-stock"
            st.markdown(f"<div style='background:white; padding:15px; border-radius:10px; border:1px solid #eee; margin-bottom:10px;'><div style='font-weight:bold; font-size:16px;'>{rk}위. {row['name']} <span class='{badge_class}'>{row['type']}</span></div><div style='color:#666; font-size:13px;'>{row['brand']} | {format(row['price'],',')}원 (권장소비자가)</div></div>", unsafe_allow_html=True)
    ...
    try:
        ...
        if q8_score >= 4:
            guide_text += "• 📱 <b>근거리 피로 호소:</b> 디지털/근거리 작업 시 불편함이 큽니다. <b>기능성(피로완화)</b>나 <b>다초점 렌즈</b> 필요성을 설명해 주세요."
    except: pass
